...
int main() {
	...
	do {
		...
		//SUELO
		glStencilOp(GL_REPLACE, GL_KEEP, GL_REPLACE); //( si falla el stencil, si falla z, si no falla ninguno)		
		glStencilFunc(GL_NEVER, 1, 0xFF); //operacion, valor, màscara	
		drawFloor(true);		
		//el stencil siempre falla, entonces lo reemplaza por 1	
		
		//SOMBRA
		glStencilOp(GL_KEEP, GL_INCR, GL_REPLACE);		
		glDepthFunc(GL_NEVER); //hago que falle el z
		glStencilFunc(GL_EQUAL, 1, 0xFF); //operacion, mascara, binario		
		drawObject(shadow);	//sombra GUIA
		//La sombra guia no se dibuja ya que no pasa el stencil pero si queda guardado con la mascara 2		
		glDepthFunc(GL_LESS);		
		...
		drawObject(identity); //se dibuja el pollo principal
		drawLight();
