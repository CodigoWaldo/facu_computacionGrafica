...
cameraPos setViewAndProjectionMatrixes(const Car &car) {
	...
	//Se crea un auxCam tipo struct que contiene las posiciones de la cámara en 'x','y' y 'z'.	
	cameraPos auxCam;
	...
	if (play) {
		if (top_view) {
			...
			/*
			para que la cámara vea al modelo desde arriba, se le asigna 30 unidades de altura, la posición del auto 
			y un sistema de ejes coordenados de seguimiento.
			
			glm::lookat(eye camera position, target position, vector de seguimiento);
			
			La cámara debe coincidir con los ejes del auto, esto se logra utilizando el ángulo de giro del mismo y
			descomponiendo el vector dirección en sus ejes 'x' e 'y'
			*/	
			...
			//En este caso la cámara no gira a la par del auto, solo lo persigue de atrás por lo que debe
			//modificarse su posición en el espacio.
    ...
    */	
	...
	if (fwing.show or play) {
		renderPart(car,fwing.models,glm::mat4( 0.0f, 0.0f, 0.5f, 0.0f,
											   0.0f, 0.5f, 0.0f, 0.0f,
											   -0.5f, 0.0f, 0.0f, 0.0f,
											   0.95f, 0.10f, 0.0f, 1.0f )); // aleron frontal: rotar 90 grados, espejar 
	}
	...
	do {
		...
		window.ImGuiDialog("CG Example",[&](){
			...
			if (ImGui::TreeNode("car and cam")) {
				...
				ImGui::LabelText("","ang: %f",car.ang*180/3.14f);
