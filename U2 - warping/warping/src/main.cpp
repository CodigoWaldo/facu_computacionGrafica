...
glm::vec3 warpPoint(const Delaunay &delaunay0, const Delaunay &delaunay1, glm::vec3 p) {
	...
	int TrianguloNum = 0;
	TrianguloNum = delaunay0.enQueTriangulo(p);                     // 1 	
	Triangulo T = ( delaunay0.getTriangulos() )[TrianguloNum];		// 2 
	
	/* 	
	1: averigua en qué triangulo de la malla se encuentra p	
	2: se crea una variable de T tipo Struct Triangulo y se guarda el triangulo	
	
	----------------------------------------		
	Se calcula el peso que ejercen los vertices del triangulo de la malla sobre el punto p */
	vector<glm::vec3> v = delaunay0.getPuntos();			// 1
	Pesos pesos = calcularPesos(v[T[0]],v[T[1]],v[T[2]],p);	// 2	
	/* 	
	1: vector con todos los vertices del triangulo(malla)
	2: Se calculan los pesos que influyen a p a partir de los los vertices de la malla y se guarda en pesos {p0,p1,p2}
	...
	*/
	...
	vector<glm::vec3> vNew = delaunay1.getPuntos();			// 1 
	
	p.x = ( pesos[0]* vNew[T[0]].x ) + ( pesos[1]* vNew[T[1]].x ) + ( pesos[2]* vNew[T[2]].x );  // 2
	p.y = ( pesos[0]* vNew[T[0]].y ) + ( pesos[1]* vNew[T[1]].y ) + ( pesos[2]* vNew[T[2]].y );  // 3
	
	/* 	
	1: vector con todos los vertices del triangulo(malla) modificados
	2: Coordenada X nueva del punto p 
	3: Coordenada Y nueva del punto p
	...
	Ya que solamente el usuario mueve un vertice a la vez, solamente un producto variará por modificación
	
	producto n = (peso calculado)*(valor de la coordenada nueva de un vertice de la malla en x o y)
	p.eje = (producto 1)+(producto 2)+(producto 3)
	----------------------------------------
	*/
